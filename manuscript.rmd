...
## Title  
Sputum quantity and adverse events during mechanical ventilation      
  
## Abbreviated Title  
Sputum and ventilator-associated complications  
...
4 - Division of Infectious Diseases, Mount Sinai School of Medicine, New York, NY  
...
#### Background  
Tracheal intubation and mechanical ventilation provide essential support for patients with respiratory failure, but the course of mechanical ventilation may be complicated by adverse events. The Center for Disease Control and Prevention (CDC)'s surveillance definitions of adverse ventilator-associated events (VAE) categorize events as infection-associated or not. We sought to understand how the quantity of sputum produced by mechanically ventilated patients relates to the onset of VAE and whether VAE are categorized as infection-associated or not.  
  
#### Methods  
We performed a case-crossover study including 87 patients with VAE and evaluated 848 days in the pre-VAE period at risk for VAE. Sputum quantity at time of VAE and in the immediate pre-VAE period was compared to sputum quantity in the preceding period. We used the number of as-needed subglottic suctioning events performed per calendar day to quantify sputum production. We used CDC surveillance definitions for VAE and to categorize whether events were infection-associated or not.  
  
#### Results  
Sputum quantity is greater in the period immediately prior to VAE, compared to the preceding period. However, sputum quantity does not discriminate well between VAE that are categorized as infection-associated and VAE that are not.  
  
#### Conclusions  
Subglottic suction frequency may serve as a valuable marker of sputum quantity, which is associated with risk for VAE. However, our results require validation in a broader population of mechanically ventilated patients and intensive care settings.  
...
In 2013, the Centers for Disease Control and Prevention (CDC)'s National Healthcare Safety Network (NHSN) implemented a new surveillance paradigm for adverse ventilator-associated events (VAE), which advanced a broader, objective definition of ventilator-associated complications (VAC) [@Klompas2013-tj; @Magill2013-kw; @Magill2014-xi]. VAC are now defined as an increase of daily minimum positive end-expiratory pressure (PEEP) of at least 3cm H2O or an increase in daily minimum fraction of inspired oxygen (FiO2) by at least 20%, after at least two calendar days of stable or decreasing requirements. There are three tiers of VAC: (1) VAC without signs of infection; infection-associated VAC (IVAC), which are accompanied by leukocytosis/leukopenia or fever/hypothermia, as well as altered antibiotic treatment; and (3) possible ventilator-associated pneumonia (PVAP), which are IVAC with sputum studies suggestive of VA-LRTI. The NHSN definitions of VAE are well validated to predict ventilator-associated morbidity and mortality [@Kobayashi2017-ue; @Meagher2019-py]. VAE criteria also form the basis for promising novel antimicrobial stewardship strategies [@Klompas2017-bw].  
   
However, current VAE surveillance definitions of VAE capture a different population than clinically defined VA-LRTI, which are typically diagnosed by physical examination and radiographic findings, as well as microbiologic test results [@Fan2016-np; @Klompas2014-uv; @Ramirez-Estrada2018-bf; @Stevens2014-zg]. More than 50% of traditionally-defined VA-LRTI fail to meet VAE criteria [@Klompas2019-tt]. The best validated interventions to reduce VAE involve minimizing sedation and careful monitoring of volume status, rather than interventions specifically targeted to infection prevention [@Cocoros2016-md; @Lewis2014-of; @Klompas2015-nj; @Klompas2015-lh; @Rello2019-jl; @Liu2019-fm; @Vaewpanich2019-dr].  
  
Sputum production is a physical examination finding that is often used in the clinical adjudication of VA-LRTI, but its relationship to VAE defined by CDC NHSN criteria is unknown. It is challenging to quantify sputum production, but a marker of sputum production is the frequency with which subglottic suction is performed. Subglottic suction is a procedure performed on an as-needed basis by nurses, respiratory therapists, and physicians to clear lower airway secretions [@Caroff2016-ef]. We sought to evaluate the relationship between the quantity of sputum produced and VAE. Specifically, we sought to define how sputum quantity at the time of VAE relates to sputum quantity before VAE, and to define whether sputum quantity discriminates between VAC and IVAC/VAP, using the frequency of subglottic suction events as a measure of sputum quantity. Here we report the results of a case-crossover study designed to address these questions.   
...
#### Study Design  
We performed a case-crossover study to define the relationship between the quantity of sputum production (primary exposure), measured as the frequency with which nurses and respiratory therapists chose to perform as-needed subglottic suctioning, and the occurrence of VAE (primary outcome). We also analyzed whether VAE were categorized as VAC without suspected infection or as IVAC. For the primary anlysis, the day of VAE was the case period; the days prior to VAE were the control periods.   
  
#### Study Setting    
We evaluated 90 consecutive VAE that occurred across 87 unique subjects, over 3.5 years in the medical intensive care unit (MICU) of the Hospital of the University of Pennsylvania (HUP), a tertiary academic medical center. All patients who depend on mechanical ventilation at HUP were monitored for VAE, and all VAE were reported to NHSN. VAC and IVAC were captured by review of HUP NHSN reporting records. The study was approved by the University of Pennsylvania Institutional Review Board (protocol #824613).  
...
Data, model code, and code used to produce the figures is available at github.com/bjkellyio.  
...
#### Sputum quantity increases prior to VAE.  
...
### Discussion  
Using a case-crossover design, we investigated the relationship between sputum production during mechanical ventilation and adverse ventilator-associated events (VAE) with the goal of understanding whether sputum quantity reflects underlying lung disease that will result in increased oxygen (FiO2 and PEEP) requirement. We quantified sputum production as the number of subglottic suction events per calendar day; we categorized VAE using the CDC's NHSN definitions of VAC and IVAC. In this cohort of medical intensive care patients with VAE, sputum quantity was associated with both IVAC and VAC. Though the association with IVAC was stronger, sputum quantity did not discriminate well between infection-associated and non-infectious VAE. Secondary analyses revealed other clinical variables, prominently AST and ALT, that better discriminated IVAC from VAC in this cohort.         
  
The relationship between sputum quantity and VAE is of interest because sputum quantity is often clinically interpreted as a sign of VA-LRTI, and the relationship between VAE and VA-LRTI remains unclear [@Fan2016-np; @Klompas2014-uv; @Ramirez-Estrada2018-bf; @Stevens2014-zg]. The finding that sputum quantity is higher at the time of VAE and immediately before VAE, relative to the earlier pre-VAE period, validates sputum quantity as a physical examination sign associated with risk for worsening respiratory failure. However, sputum quantity did not discriminate well between IVAC and VAC. Greater sputum quantity was associated with VAC without fever or leukocytosis, as well as with IVAC. There are several possible explanations for the discordance between sputum quantity and the NHSN criteria used to discriminate infection-associated adverse events. One possible explanation is that the number of subglottic suction events does not reliably represent the quantity of sputum produced; misclassification of sputum production may result in poor discrimination between IVAC and VAC. Another possible explanation is that NHSN IVAC criteria, which are known to exclude a large proportion of clinically diagnosed VA-LRTI [@Klompas2019-tt], exclude true VA-LRTI events that would drive a stronger association with sputum quantity. A third possible explanation is that sputum production, though commonly interpreted as a sign of infection, is nonspecific, or that sputum quality (e.g., purulence) is more specific than sputum quantity.  
...
#### Table 1. Comparison of clinical characteristics among subjects high or low sputum production at onset of mechanical ventilation. Categorical variables are described as number (%); continuous variables are described as median (IQR). Categorical variables are compared with chi-square test or Fisher's exact test; continuous variables are compared with Wilcoxon rank-sum test.  				
...
![](figs/main/p_binomial_combined.png){width=750px, align='center'}  
...
![](figs/main/p_ivac_yn_combined.png){width=750px, align='center'}  
...
![](figs/supplement/weibull_survival_vae_vs_admit_ss.png){width=250px, align='center'}  
...
![](figs/supplement/p_ivac_vs_vac_explore.png){width=750px, align='center'}  
...
The association between the sum of AST and ALT and probability of IVAC versus VAC are shown, stratied by number of pressors and number of antibiotics. Best estimates of the associations are represented with a dark line, surrounded by bands of posterior certainty.  
  
  
#### Supplemental Figure 3  
  
![](figs/supplement/p_culture_ss_fever.png){width=750px, align='center'}  
